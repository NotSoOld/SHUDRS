		...
		public bool? this[int i, int j] {
			...
			get {
				if (i >= len || j >= len || i < 0 || j < 0) {
					...
					Debug.LogErrorFormat(
						"Array index for 'get' is out of range! Indexes must be between '(0, 0)' and '({0}, {0}), was ({1}, {2})",
						(len - 1), i, j
					);
#endif
					return null;
					...
					/// (*******1 % 2 == true, *******0 % 2 == false).
					return Convert.ToBoolean((arr[((i * len + j) / 8)] >> ((i * len + j) % 8)) % 2);
				}
			}
			...
			set {
				if (i >= len || j >= len || i < 0 || j < 0) {
#if UNITY_EDITOR
					Debug.LogErrorFormat(
						"Array index for 'set' is out of range! Indexes must be between '(0, 0)' and '({0}, {0}), was ({1}, {2})",
						(len - 1), i, j
					);
#endif
				} else {
					...
					if (value.Value) {
						arr[((i * len + j) / 8)] |= (byte)(1 << ((i * len + j) % 8));
					} else {
						arr[((i * len + j) / 8)] &= (byte)(~(1 << ((i * len + j) % 8)));
					}
				}
